...
//    private static final String TAG = "MissionDetailsActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mission = getIntent().getBundleExtra(STR_BUNDLE_FOR_MISSION).getParcelable(STR_MISSION);
...
//        int totalLearnedItemsNum =8700;
    ...
    public void toAllItemsDetails(View view){
        ...
        intentToItems.putExtra(STR_MISSION,mission);
    ...
    public void toAllGroupsDetails(View view){
        ...
        intentToGroups.putExtra(STR_MISSION,mission);
    ...
    * */
    public void fastLearn(View view){
        ...
        noMoreL_TipBox = sharedPreferences.getBoolean(SP_STR_NO_MORE_TIPS, false);
        isFastLearnUseOrder = sharedPreferences.getBoolean(SP_STR_IS_ORDER,true);

        Toast.makeText(this, "MissionDetail页快速学习按钮", Toast.LENGTH_SHORT).show();
        if(!noMoreL_TipBox) {
         //并未设置“不再显示对话框”
            FragmentTransaction transaction = getFragmentManager().beginTransaction();
            Fragment prev = getFragmentManager().findFragmentByTag(FG_STR_FAST_LEARN);
            ...
            dfg.show(transaction, FG_STR_FAST_LEARN);
    ...
    public void fastRePick(View view){
        //检测sp中关于目标Dfg的特别设置
        noMoreR_TipBox = sharedPreferences.getBoolean(SP_STR_NO_MORE_TIPS, false);
        fastRePickManner = sharedPreferences.getInt(SP_FAST_R_MANNER,DEFAULT_MANNER_TT);
        if(!noMoreR_TipBox) {
            //未设置不显示对话框
            FragmentTransaction transaction = getFragmentManager().beginTransaction();
            Fragment prev = getFragmentManager().findFragmentByTag(FG_STR_FAST_RE_PICK);
            ...
            dfg.show(transaction, FG_STR_FAST_RE_PICK);
        }else {
            //直接根据默认设置开始
    ...
    public void onButtonClickingDfgInteraction(int dfgType, Bundle data) {
        ...
        intentToPFL.putExtra(STR_TABLE_SUFFIX, mission.getTableItem_suffix());//通用数据

        switch (dfgType) {
            case FAST_LEARN:
            if (data != null) {
                boolean useOrder = data.getBoolean(STR_IS_ORDER);
                if(!useOrder){
                    learningType = LEARNING_AND_CREATE_RANDOM;
                }
                //对传出的全局设置进行修改。
                //（一）默认方式
                int defaultManner = data.getInt(STR_DEFAULT_MANNER_SETTINGS);
                ...
                //（二）是否继续显示对话框
                noMoreL_TipBox = data.getBoolean(STR_NO_MORE_BOX, false);
                if (noMoreL_TipBox) {
                    editor.putBoolean(SP_STR_NO_MORE_TIPS, true);
                }/*else {
            ...
            intentToPFL.putExtra(STR_LEARNING_TYPE, learningType);
            intentToPFL.putExtra(STR_MISSION_ID,mission.getId());//LC特别数据
            ...
            case FAST_RE_PICK:
                ...
                if (data != null) {
                    //对传出的全局设置进行修改。
                    //（一）默认方式（快速复习）
                    int defaultRpManner = data.getInt(STR_DEFAULT_MANNER_R_SETTINGS);
                    if (defaultRpManner != DEFAULT_MANNER_UNDEFINED) {
                        editor.putInt(STR_FAST_R_MANNER, defaultRpManner);
                    } //另一种是未设置，则不做更改即可

                    //（二）是否继续显示对话框（快速复习）
                    noMoreL_TipBox = data.getBoolean(STR_NO_MORE_BOX, false);
                    if (noMoreL_TipBox) {
                        editor.putBoolean(SP_NO_MORE_R_TIPS, true);
                    }/*else {
                        editor.putBoolean("NO_MORE_TIPS",false);//【从“不开启”到“开启”的改变则应在其他页面处理，如设置界面。】
                    }*/
                    editor.apply();

                    intentToPFL.putExtra(STR_PRIORITY_SETTING,defaultRpManner);//LGN的特别数据，要在if内设置

                }

                //主要任务：跳转到学习准备页（该页负责根据学习类型进行数据准备）
                intentToPFL.putExtra(STR_LEARNING_TYPE, learningType);
                intentToPFL.putExtra(STR_MISSION_ID,mission.getId());
                this.startActivity(intentToPFL);
